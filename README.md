...
| [`simpl-sd-tooling/`](./simpl-sd-tooling/README.md) | SD Tooling (`simpl-sd-ui` + `sdtooling-api-be` + `sdtooling-validation-api-be`) | Verified end-to-end 2026-07-06: 3 seeded schemas served, SHACL-driven 8-step wizard renders, full API authoring loop green (enrich → finalize/sign → publish) against one WireMock standing in for all peers | The Creation Wizard a provider uses to author, SHACL-validate, sign, and publish Self-Descriptions. Astro SSR UI (BFF pattern) + two Spring Boot backends, **no datastores at all** - schemas are static TTL/JSON pairs on a volume (replacing the schema-sync-adapter chain: Postgres + Kafka + IAA not needed). **No auth** (`WEB_MVC_BEARER_TOKEN_REQUIRED=false` + empty Keycloak env in the UI; validation-api has no security upstream at all). One WireMock stubs authentication-provider, connector-adapter (echoes the SD + injects the EDC fields the real adapter adds), asset-orchestrator, vc-issuer, catalogue-adapter, federated-catalogue, and the tier2 clients' mTLS keypair bootstrap (throwaway self-signed key, generated + gitignored). Upstream gotchas documented: `${env.PROJECT_RELEASE_VERSION}` poms, any-non-empty `USE_MOCK_APIS` enables UI mocks. |

*Status dates are per-stack verification dates against upstream `main` at that time. Matrix last reviewed: 2026-07-06.*
